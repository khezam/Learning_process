class HashTable:
    ...
    def second_hash(self, home, i):
        """To ensure the prob sequence includes all table entries
            the constant factor c and the table size must be
            relatively prime. Therefore less collision will occure vs without a constant
        """
        c = 3
        m = len(self.table)
        slot = (home + i*c) % m
